...
namespace ConsoleArenaGame
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (gameEngine.CheckWeapons()){gameEngine.Fight();}
            

            Console.WriteLine();
            Console.WriteLine($"And the winner is {gameEngine.Winner}");

            GameEngine gameEngine_1 = new GameEngine()
            {
                HeroA = new Sorceress("Elza", 10, 20, new Staff("Sudden Death"))
            };
            gameEngine_1.HeroB = new Necromancer(gameEngine_1, "Darkmage", 10, 5, new Wand("Wand of darkness"));            

            if (gameEngine_1.CheckWeapons()) {gameEngine_1.Fight(); }


            Console.WriteLine();
            Console.WriteLine($"And the winner is {gameEngine_1.Winner}");
